...
It is __NOT__ recommended that the following section of R-script be run with every time this document is compiled. Running this script each time should not cause any errors but could lead to significant wait times associated with downloading the data from CEDR. The phytoplankton taxonomic counts and water quality data is relatively large and can take some time to download; therefore, this script should be run after your initial download from Git Hub, after a known update has been made, or after a significant amount of time has passed from the initial data download (~6 months). 
