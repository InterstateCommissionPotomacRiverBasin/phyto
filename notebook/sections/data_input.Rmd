...
L.V.: this is a modified version of data_aquisition created, by Luke Vawter, for use with VA_ODU data. DO NOT USE THIS SECTION if you are pulling data from CEDR.  Use the master branch of phyto2 which uses  data_aquisition instead.

L.V.: as in Zach's code, the root directory is set as the phyto project base directory. Next a new directory is created,if it doesn't already exist, for storing phyto2 specific csv that this modified r markdown generates.Currently this only includes VA ODU data, but Maryland will soon follow. These files are input into the pipeline via a csv, rather than the CEDR api.  All modifications to the code are meant to address variations in ofrmatting that this new source presents. 
...
L.V.: currently we don't need this code for our data as we aren't accessing the cedr json api, but I've left it in as a reference.
```{r}
...
L.V.: This code replaces the CEDR information download with data from the most recent VA excel sheet provided by Mike Malonee:
```{r}
...
L.V.: Altered director name to phytoplankton2 and output file name for VA_ODU.  original file name was cedr_phyto_taxa.csv
```{r}
dir.create(file.path(project.dir, "data/phytoplankton2/"),
           recursive = TRUE, showWarnings = TRUE)
phyto.df %>% 
...
L.V.:Download all of the monitoring event data associated with the collection of phytoplankton from VA ODU spreadsheet. original filename was cedr_phyto_event.csv
```{r}
...
L.V: The next section is unchanged
